// suma4horasIngresos.cjs
/*
instaslar ts-node con TypeScript: npm install --save-dev ts-node typescript @types/node
en package.json poner  "fix-dates": "ts-node scripts/suma4horasIngresos.cjs"
...
const carteraIngreso = mongoose.connection.collection('carteraIngreso');
...
const filter = { tzFixed: { $ne: true } }; 
  ...
  const toFix = await carteraIngreso.countDocuments(filter);
  ...
  const result = await carteraIngreso.updateMany(
    filter,
    [
      {
        $set: {
